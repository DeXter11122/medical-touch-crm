...
.service-card{background:white;border-radius:16px;padding:20px;cursor:pointer;border:1px solid #eee;transition:0.3s;}
...
.service-duration{color:#999;font-size:12px;margin-top:5px;}
...
.chatbot-btn{position:fixed;bottom:30px;right:30px;width:60px;height:60px;background:#ff6b9d;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:30px;box-shadow:0 5px 20px rgba(0,0,0,0.2);z-index:1000;}
.chatbot-window{position:fixed;bottom:110px;right:30px;width:350px;height:450px;background:white;border-radius:20px;display:none;flex-direction:column;box-shadow:0 10px 40px rgba(0,0,0,0.2);z-index:1000;}
...
.warning{color:red;font-size:12px;display:none;}
@media(max-width:768px){.hero h1{font-size:32px;}.chatbot-window{width:300px;right:10px;}}
</style>
</head>
<body>
...
<div class="tabs"><button class="tab active" onclick="switchTab('nails')">💅 Nails</button><button class="tab" onclick="switchTab('lashes')">👁️ Lashes</button><button class="tab" onclick="switchTab('skincare')">💆 Skincare</button><button class="tab" onclick="switchTab('wax')">🕯️ Wax</button><button class="tab" onclick="switchTab('book')">📅 Book</button></div>
<div class="container">
...
<form id="bookingForm"><input type="text" id="custName" placeholder="Full Name" required><input type="tel" id="custPhone" placeholder="Phone" required><input type="email" id="custEmail" placeholder="Email"><select id="serviceSelect" required><option value="">Select Service</option></select><input type="datetime-local" id="appointmentDate" required><div id="slotWarning" class="warning">⚠️ This time is already booked</div><button type="submit" class="submit-btn">Confirm Booking</button></form></div></div>
</div>
<footer><p>Medical Touch | Bakaata - Ain W ZEIN Road | 81023625</p></footer>
<div class="chatbot-btn" onclick="toggleChatbot()">🤖</div>
<div class="chatbot-window" id="chatbotWindow"><div class="chatbot-header">🤖 Medical Touch AI</div><div class="chatbot-messages" id="chatMessages"><div class="message bot-message">Hello! Ask me about nails, lashes, skincare, wax, or prices!</div><div><button class="quick-btn" onclick="sendQuick('nails')">💅 Nails</button><button class="quick-btn" onclick="sendQuick('lashes')">👁️ Lashes</button><button class="quick-btn" onclick="sendQuick('skincare')">💆 Skincare</button><button class="quick-btn" onclick="sendQuick('wax')">🕯️ Wax</button><button class="quick-btn" onclick="sendQuick('price')">💰 Prices</button><button class="quick-btn" onclick="sendQuick('book')">📅 How to book</button></div></div><div class="chatbot-input"><input type="text" id="chatInput" placeholder="Ask me..." onkeypress="if(event.key==='Enter')sendChat()"><button onclick="sendChat()">Send</button></div></div>
<script>
let services = [];
fetch('/api/services').then(r=>r.json()).then(s=>{services=s;displayServices(s);populateSelect(s);});
function displayServices(s){const cats={Nails:'nailsGrid',Lashes:'lashesGrid',Skincare:'skincareGrid',Wax:'waxGrid'};for(let[cat,id] of Object.entries(cats)){let filtered=s.filter(x=>x.category===cat);let html='';filtered.forEach(service=>{html+=`<div class="service-card" onclick="bookService('${service.name}')"><div class="service-name">${service.name}</div><div class="service-price">$${service.price}</div><div class="service-duration">⏱️ ${service.duration} min</div><div style="color:#ff6b9d;margin-top:10px;">Click to book →</div></div>`;});document.getElementById(id).innerHTML=html||'<p>Loading...</p>';}}
...
async function sendQuick(topic){let q='';if(topic==='nails')q='Tell me about nail services';if(topic==='lashes')q='Tell me about lash services';if(topic==='skincare')q='Tell me about skincare treatments';if(topic==='wax')q='Tell me about wax services';if(topic==='price')q='What are your prices?';if(topic==='book')q='How do I book an appointment?';addMessage(q,'user');const r=await fetch('/api/ai/customer-chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question:q})});const d=await r.json();addMessage(d.answer,'bot');}
...
<!DOCTYPE html>
<html>
<head><title>Medical Touch Admin</title><meta name="viewport" content="width=device-width, initial-scale=1"><link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box;}body{font-family:'Poppins',sans-serif;background:#f0f2f5;}
.sidebar{width:260px;background:#1a1a2e;color:white;position:fixed;height:100%;padding:25px;overflow-y:auto;}
...
.profit-detail{margin-top:15px;padding:12px;background:#fef8f9;border-radius:10px;display:none;}
.floating-bell{position:fixed;bottom:30px;right:30px;width:60px;height:60px;background:#ff6b9d;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:28px;box-shadow:0 5px 20px rgba(0,0,0,0.2);z-index:1000;}
...
.notif-popup{position:fixed;bottom:110px;right:30px;width:320px;background:white;border-radius:15px;display:none;box-shadow:0 5px 20px rgba(0,0,0,0.2);z-index:1000;}
.notif-popup.show{display:block;}
.notif-header{background:#ff6b9d;color:white;padding:12px;border-radius:15px 15px 0 0;}
...
<div id="services" class="section"><h2>💅 Services</h2><div><input type="text" id="newName" placeholder="Service"><input type="number" id="newPrice" placeholder="Price"><select id="newCat"><option>Nails</option><option>Lashes</option><option>Skincare</option><option>Wax</option></select><input type="number" id="newCost" placeholder="Material Cost"><button onclick="addService()">Add</button></div><div id="serviceTable"></div></div>
<div id="ai" class="section"><div class="ai-box"><h2>🤖 AI Assistant</h2><p>Ask about profits, predictions, or popular services</p><input type="text" id="aiQuestion" placeholder="How much profit?"><button onclick="askAI()">Ask</button><div id="aiResponse" class="ai-response"></div></div></div></div>
...
function showPopular(n){const d=document.getElementById('popularDetail');const p=popularData.find(x=>x.name===n);if(p)d.innerHTML=`🎯 ${p.name}: ${p.count} bookings | Net: $${p.netProfit||0}`;d.style.display='block';setTimeout(()=>d.style.display='none',4000);}
async function loadMaterials(){const r=await fetch('/api/materials');const d=await r.json();let h='<table><th>Category</th><th>Monthly Cost</th><th>Items</th><th>Action</th></tr>';for(let cat in d.materials){h+=`<tr><td><strong>${cat}</strong></td><td><input type="number" id="cost_${cat}" value="${d.materials[cat].cost}" style="width:80px"> $</td><td>${d.materials[cat].items.join(', ')}</td><td><button onclick="updateCost('${cat}')">Update</button></td>`}h+='</table>';document.getElementById('materialsGrid').innerHTML=h;document.getElementById('profitSummary').innerHTML=`<h3>💰 Financial Summary</h3><p>Total Revenue: $${d.totalRevenue} | Materials: $${d.totalMaterialCost} | <strong>Net Profit: $${d.netProfit}</strong></p>`;}
...
async function loadCustomers(){const r=await fetch('/api/customers');const c=await r.json();let h='<table><th>Name</th><th>Phone</th><th>Email</th><th>Visits</th><th>Action</th></tr>';c.forEach(cust=>{h+=`<tr><td>${cust.name}</td><td>${cust.phone}</td><td>${cust.email||'-'}</td><td>${cust.visits||0}</td><td><button class="delete-btn" onclick="deleteCustomer('${cust.id}')">Delete</button></td>`});h+='</table>';document.getElementById('customerTable').innerHTML=h;}
async function loadAppointments(){const r=await fetch('/api/appointments');const a=await r.json();let h='<table><th>Customer</th><th>Service</th><th>Time</th><th>Status</th><th>Action</th></tr>';a.forEach(app=>{h+=`<tr><td>${app.customer_name}</td><td>${app.service}</td><td>${app.datetime}</td><td><select onchange="updateStatus('${app.id}',this.value)"><option ${app.status==='pending'?'selected':''}>pending</option><option ${app.status==='confirmed'?'selected':''}>confirmed</option><option ${app.status==='completed'?'selected':''}>completed</option></select></td><td><button class="delete-btn" onclick="deleteAppointment('${app.id}')">Cancel</button></td>`});h+='</table>';document.getElementById('appointmentTable').innerHTML=h;}
async function loadServices(){const r=await fetch('/api/services');const s=await r.json();let h='<table><th>Service</th><th>Price</th><th>Duration</th><th>Category</th><th>Material</th><th>Action</th></tr>';s.forEach(serv=>{h+=`<tr><td>${serv.name}</td><td>$${serv.price}</td><td>${serv.duration}min</td><td>${serv.category}</td><td>$${serv.material_cost||0}</td><td><button class="delete-btn" onclick="deleteService('${serv.id}')">Delete</button></td>`});h+='</table>';document.getElementById('serviceTable').innerHTML=h;}
...
def customer_chat():
    ...
    if 'nail' in q: a = "💅 We offer Gel, Acrylic, Polygel, Dip Powder. Full sets $30-40. Manicure $10. Book now!"
    elif 'lash' in q: a = "👁️ Classic $35, Volume $38, Mega Volume $45. Refills $25. Lasts 3-4 weeks!"
    elif 'skin' in q or 'facial' in q or 'meso' in q: a = "💆 Facials $35-65, Mesotherapy $35-100, HIFU $100. Results guaranteed!"
    elif 'wax' in q: a = "🕯️ Full body $45, Full legs $17, Bikini $23. Pain-free experience!"
    elif 'price' in q: a = "💰 Nails: $10-40, Lashes: $35-45, Skincare: $35-200, Wax: $3-45"
    elif 'book' in q: a = "📅 Tap 'Book' tab, select service, pick date/time. We'll confirm by SMS!"
    elif 'location' in q or 'address' in q: a = "📍 Bakaata - Ain W ZEIN Road. Call 81023625 for directions!"
    else: a = "✨ Medical Touch: Nails, Lashes, Skincare, Waxing. Ask about any service!"
...
def admin_stats():
    ...
    return jsonify({'profit': {'today': t_rev, 'week': w_rev, 'month': m_rev, 'year': y_rev, 'todayNet': t_rev - t_mat, 'weekNet': w_rev - w_mat, 'monthNet': m_rev - m_mat, 'yearNet': y_rev - y_mat, 'todayCount': t_cnt, 'weekCount': w_cnt, 'monthCount': m_cnt, 'yearCount': y_cnt}, 'stats': [{'title': 'Customers', 'value': len(data['customers'])}, {'title': 'Appointments', 'value': len(apps)}, {'title': 'Pending', 'value': sum(1 for a in apps if a.get('status') == 'pending')}, {'title': 'Completed', 'value': sum(1 for a in apps if a.get('status') == 'completed')}], 'popular': popular, 'recent': apps[-10:] if apps else []})
...
def ask_ai():
    ...
    else: a = f"💡 You have {len(data['customers'])} customers, {len(apps)} appointments, ${net} profit. Ask about profits or predictions!"
    return jsonify({'answer': a})
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("✨ MEDICAL TOUCH CRM v3.0 - FULL VERSION WORKING! ✨")
    print("="*60)
